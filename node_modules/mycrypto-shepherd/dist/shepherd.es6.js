...
import { take, takeEvery, apply, flush, put, actionChannel, call, cancelled, race, select, spawn, cancel, fork, all } from 'redux-saga/effects';
...
const isStaleCall = (state, callId) => {
    const call$$1 = getProviderCallById(state, callId);
    return !call$$1 || !call$$1.pending;
};
...
const handleProviderCallSucceeded$1 = (state, { payload }) => {
    const call$$1 = state[payload.providerCall.callId];
    if (!call$$1 || !call$$1.pending) {
        throw Error(`Pending provider call not found ${call$$1 ? call$$1.callId : ''}`);
    }
...
const handleProviderCallPending = (state, { payload }) => {
    const call$$1 = state[payload.callId];
    // a duplicate check that makes sure the incoming call is either new or a retry call
    if (call$$1 && call$$1.numOfRetries === payload.numOfRetries) {
...
const addProviderIdToCall = (call$$1, providerId) => (Object.assign({}, call$$1, { providerId }));
const trackTime = () => {
...
class ProviderChannels {
    ...
    *take(providerId) {
        const channel$$1 = this.getChannel(providerId);
        const action = yield apply(channel$$1, channel$$1.take);
        return action;
    }
    *cancelPendingCalls() {
    ...
    getChannel(providerId) {
        const channel$$1 = this.providerChannels[providerId];
        if (!channel$$1) {
