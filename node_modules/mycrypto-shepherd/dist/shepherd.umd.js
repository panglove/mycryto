(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('bn.js'), require('jsonschema'), require('crypto'), require('btoa'), require('url-search-params'), require('redux'), require('redux-saga/effects'), require('redux-saga'), require('remote-redux-devtools')) :
    typeof define === 'function' && define.amd ? define(['exports', 'bn.js', 'jsonschema', 'crypto', 'btoa', 'url-search-params', 'redux', 'redux-saga/effects', 'redux-saga', 'remote-redux-devtools'], factory) :
    (factory((global.shepherd = {}),global.bn.js,global.jsonschema,global.crypto,global.btoa,global['url-search-params'],global.redux,global['redux-saga/effects'],global['redux-saga'],global['remote-redux-devtools']));
}(this, (function (exports,BN,jsonschema,crypto,btoa,URLSearchParams,redux,effects,createSagaMiddleware,remoteReduxDevtools) { 'use strict';

    BN = BN && BN.hasOwnProperty('default') ? BN['default'] : BN;
    btoa = btoa && btoa.hasOwnProperty('default') ? btoa['default'] : btoa;
    URLSearchParams = URLSearchParams && URLSearchParams.hasOwnProperty('default') ? URLSearchParams['default'] : URLSearchParams;
    ...
    ***************************************************************************** */
    ...
    const handleBalancerAuto = (state, _) => (Object.assign({}, state, { manual: false }));
    const handleBalancerManual = (state, { payload }) => (Object.assign({}, state, { manual: payload.providerId }));
    const balancerConfigReducer = (state = INITIAL_STATE, action) => {
        switch (action.type) {
            ...
            case BALANCER.NETWORK_SWITCH_SUCCEEDED:
                return Object.assign({}, state, { network: action.payload.network, networkSwitchPending: false });
            case BALANCER.NETWORK_SWTICH_REQUESTED:
                return Object.assign({}, state, { networkSwitchPending: true });
            case BALANCER.SET_PROVIDER_CALL_RETRY_THRESHOLD:
                return Object.assign({}, state, { providerCallRetryThreshold: action.payload.threshold });
            default:
                return state;
        }
    };
    ...
    const getOnlineProviders = (state) => {
        ...
        return Object.entries(providers).reduce((accu, [curProviderId, curProvider]) => {
            if (isOnline(curProvider)) {
                return Object.assign({}, accu, { [curProviderId]: curProvider });
            }
            return accu;
        }, initialState);
    };
    ...
    const handleWorkerKilled = (state, { payload: { providerId, workerId } }) => {
        ...
        const nextProviderProviderStatsState = Object.assign({}, providerToChange, { currWorkersById: providerToChange.currWorkersById.filter(id => id !== workerId) });
        return Object.assign({}, state, { [providerId]: nextProviderProviderStatsState });
    };
    const handleWorkerSpawned = (state, { payload: { providerId, workerId } }) => {
        ...
        const nextProviderProviderStatsState = Object.assign({}, providerToChange, { currWorkersById: [...providerToChange.currWorkersById, workerId] });
        return Object.assign({}, state, { [providerId]: nextProviderProviderStatsState });
    };
    const handleProviderOnline = (state, { payload: { providerId } }) => {
        ...
        return Object.assign({}, state, { [providerId]: Object.assign({}, providerToChange, { isOffline: false }) });
    };
    const handleProviderOffline = (state, { payload: { providerId } }) => {
        ...
        return Object.assign({}, state, { [providerId]: Object.assign({}, providerToChange, { isOffline: true, requestFailures: 0 }) });
    };
    const handleProviderAdded = (state, { payload: { providerId, stats } }) => {
        if (state[providerId]) {
            throw Error(`Provider ${providerId} already exists`);
        }
        return Object.assign({}, state, { [providerId]: stats });
    };
    ...
    const handleProviderCallTimeout = (state, { payload: { providerCall: { providerId } } }) => {
        ...
        return Object.assign({}, state, { [providerId]: Object.assign({}, providerToChange, { requestFailures: providerToChange.requestFailures + 1 }) });
    };
    const handleBalancerFlush = state => Object.entries(state).reduce((obj, [providerId, stats]) => (Object.assign({}, obj, { [providerId]: Object.assign({}, stats, { requestFailures: 0 }) })), {});
    ...
    const handleWorkerProcessing = (state, { payload: { currentPayload, workerId } }) => {
        ...
        return Object.assign({}, state, { [workerId]: Object.assign({}, state[workerId], { currentPayload }) });
    };
    const handleWorkerSpawned$1 = (state, { payload }) => {
        if (state[payload.workerId]) {
            throw Error(`Worker ${payload.workerId} already exists`);
        }
        return Object.assign({}, state, { [payload.workerId]: {
    ...
    const handleProviderCallSucceeded = (state, { payload }) => {
        ...
        return Object.assign({}, state, { [workerId]: Object.assign({}, workerInst, { currentPayload: null }) });
    };
    ...
    const handleProviderCallSucceeded$1 = (state, { payload }) => {
        ...
        return Object.assign({}, state, { [payload.providerCall.callId]: Object.assign({}, payload.providerCall, { result: payload.result, error: null, pending: false }) });
    };
    const handleProviderCallFailed = (state, { payload }) => {
        ...
        return Object.assign({}, state, { [payload.providerCall.callId]: Object.assign({ error: payload.error }, payload.providerCall, { result: null, pending: false }) });
    };
    ...
    const handleProviderCallPending = (state, { payload }) => {
        ...
        return Object.assign({}, state, { [payload.callId]: Object.assign({}, payload, { error: null, result: null, pending: true }) });
    };
    const handleWorkerProcessing$1 = (state, { payload: { currentPayload } }) => {
        ...
        const nextPayload = Object.assign({}, prevPayload, { providerId: currentPayload.providerId });
        return Object.assign({}, state, { [currentPayload.callId]: nextPayload });
    };
    ...
    const handleProviderConfigAdd = (state, { payload }) => {
        ...
        return Object.assign({}, state, { [payload.id]: payload.config });
    };
    const handleProviderConfigChange = (state, { payload }) => {
        ...
        return Object.assign({}, state, { [id]: Object.assign({}, state[id], config, { supportedMethods: Object.assign({}, state[id].supportedMethods, config.supportedMethods) }) });
    };
    ...
    class RPCClient {
        constructor(endpoint, headers = {}) {
            this.decorateRequest = (req) => (Object.assign({}, req, { id: this.id(), jsonrpc: '2.0' }));
            this.call = (request) => {
    ...
    class RPCProvider {
        ...
        getTransactionByHash(txhash) {
            return this.client
                ...
                .then(({ result }) => (Object.assign({}, result, { to: result.to || '0x0', value: Wei(result.value), gasPrice: Wei(result.gasPrice), gas: Wei(result.gas), nonce: hexToNumber(result.nonce), blockNumber: result.blockNumber
        ...
        getTransactionReceipt(txhash) {
            return this.client
                ...
                .then(({ result }) => (Object.assign({}, result, { transactionIndex: hexToNumber(result.transactionIndex), blockNumber: hexToNumber(result.blockNumber), cumulativeGasUsed: Wei(result.cumulativeGasUsed), gasUsed: Wei(result.gasUsed), status: result.status ? hexToNumber(result.status) : null, root: result.root || null })));
        }
    }
    ...
    class Web3Client extends RPCClient {
        constructor() {
            ...
            this.decorateRequest = (req) => (Object.assign({}, req, { id: this.id(), jsonrpc: '2.0', params: req.params || [] }));
    ...
    const providerBalancer = redux.combineReducers({
    ...
    function* handleSubscribeToAction({ payload }) {
        ...
        const resultingAction = yield effects.take(trigger);
        callback(resultingAction);
    }
    const subscriptionWatcher = [
        effects.takeEvery(SUBSCRIBE.ACTION, handleSubscribeToAction),
    ];
    ...
    const createRetryCall = (currentCall) => {
        ...
        const nextCall = Object.assign({}, currentCall, { minPriorityProviderList: nextMinPriorityList, numOfRetries: ++currentCall.numOfRetries });
        return nextCall;
    };
    const addProviderIdToCall = (call, providerId) => (Object.assign({}, call, { providerId }));
    const trackTime = () => {
    ...
    const reduceProcessedProviders = (processedProviders, network) => {
        ...
        return processedProviders.reduce((accu, currProvider) => {
            ...
            const providerStats = Object.assign({}, accu.providerStats, curProviderStats);
            const workers = Object.assign({}, accu.workers, currProvider.workers);
            return Object.assign({}, accu, { providerStats,
                workers });
        }, initialState);
    };
    ...
    class BaseChannel {
        ...
        *take() {
            const action = yield effects.take(this.get());
            ...
            yield effects.put(subscribeToAction({
        ...
        *cancelPendingCalls() {
            const pendingCalls = yield effects.apply(this, this.getPendingCalls);
            for (const { payload } of pendingCalls) {
                yield effects.put(providerCallFlushed({
                    error: 'Call Flushed',
        ...
        *flushChannel() {
            const messages = yield effects.flush(this.get());
            this.log(`flushing`);
            this.log(messages);
            return messages;
        }
        *getPendingCalls() {
            const queuedCalls = yield effects.apply(this, this.flushChannel);
    ...
    class BalancerChannel extends BaseChannel {
        ...
        *init() {
            this.chan = yield effects.actionChannel(PROVIDER_CALL.REQUESTED, createSagaMiddleware.buffers.expanding(50));
        }
    }
    ...
    class ProviderChannel extends BaseChannel {
        ...
        *init() {
            this.chan = yield effects.call(createSagaMiddleware.channel, createSagaMiddleware.buffers.expanding(10));
        }
    }
    function* providerChannelFactory() {
        const chan = new ProviderChannel();
        yield effects.apply(chan, chan.init);
        return chan;
    }
    ...
    function* sendRequestToProvider(providerId, rpcMethod, rpcArgs) {
        try {
            const { provider, timeoutThreshold, } = yield effects.select(getProviderInstAndTimeoutThreshold, providerId);
            // make the call in the allotted timeout time
            const { result } = yield effects.race({
                result: effects.apply(provider, provider[rpcMethod], rpcArgs),
                ...
                timeout: effects.call(createSagaMiddleware.delay, isWeb3Method(rpcMethod) ? 60 * 1000 * 5 : timeoutThreshold),
            });
            if (!result) {
    ...
    function* processRequest(providerId, workerId) {
        ...
        const { payload } = yield effects.apply(providerChannels, providerChannels.take, [providerId]);
        const { rpcArgs, rpcMethod } = payload;
        const callWithPid = addProviderIdToCall(payload, providerId);
        if (yield effects.select(isStaleCall, payload.callId)) {
            logger.log(`Call ${payload.callId} is stale before processing`);
            return;
        }
        // after taking a request, declare processing state
        yield effects.put(workerProcessing({ currentPayload: callWithPid, workerId }));
        const { result, error } = yield effects.call(sendRequestToProvider, providerId, rpcMethod, rpcArgs);
        if (yield effects.select(isStaleCall, payload.callId)) {
        ...
        if (result) {
            ...
            return yield effects.put(action);
        }
        else {
            const actionParams = {
                providerCall: callWithPid,
    ...
    function* processIncomingRequests(thisId, providerId) {
        while (true) {
            yield effects.call(processRequest, providerId, thisId);
        }
    }
    function* createWorker(thisId, providerId) {
        try {
            yield effects.call(processIncomingRequests, thisId, providerId);
        }
        catch (e) {
    ...
    function* spawnWorkers(providerId, currentWorkers, maxNumOfWorkers) {
        const providerChannel = yield effects.apply(providerChannels, providerChannels.createChannel, [providerId]);
        const workers = {};
        for (let workerNumber = currentWorkers.length; workerNumber < maxNumOfWorkers; workerNumber++) {
            ...
            const workerTask = yield effects.spawn(createWorker, workerId, providerId, providerChannel);
    ...
    function* checkProviderConnectivity(providerId) {
        ...
        const timeoutThreshold = yield effects.select(getProviderTimeoutThreshold, providerId);
        try {
            const { lb } = yield effects.race({
                lb: effects.apply(provider, provider.getCurrentBlock),
                to: effects.call(createSagaMiddleware.delay, timeoutThreshold),
            });
            return !!lb;
        }
        catch (error) {
    ...
    function* processProvider(providerId, { concurrency }) {
        ...
        const providerIsOnline = yield effects.call(checkProviderConnectivity, providerId);
        const stats = makeProviderStats(timer, !providerIsOnline);
        if (!providerIsOnline) {
            yield effects.put(providerOffline({ providerId }));
        }
        const { workers, workerIds } = yield effects.call(spawnWorkers, providerId, stats.currWorkersById, concurrency);
    ...
    function* handleAddingProviderConfig({ payload: { config, id }, }) {
        const network = yield effects.select(getNetwork);
        if (network !== config.network) {
            return;
        }
        const { processedProvider, } = yield effects.race({
            processedProvider: effects.call(processProvider, id, config),
            cancelled: effects.take(BALANCER.NETWORK_SWTICH_REQUESTED),
        });
        if (!processedProvider) {
            return;
        }
        yield effects.put(providerAdded(processedProvider));
    }
    const addProviderConfigWatcher = [
        effects.takeEvery(PROVIDER_CONFIG.ADD, handleAddingProviderConfig),
    ];

    function* clearWorkers() {
        const workers = yield effects.select(getWorkers);
        for (const worker of Object.values(workers)) {
            yield effects.cancel(worker.task);
        }
    }
    function* clearAllPendingCalls() {
        yield effects.apply(providerChannels, providerChannels.cancelPendingCalls);
        yield effects.apply(balancerChannel, balancerChannel.cancelPendingCalls);
    }
    function* deleteProviderChannels() {
        yield effects.apply(providerChannels, providerChannels.deleteAllChannels);
    }
    function* clearState({ type }) {
        const isNetworkSwitch = type === BALANCER.NETWORK_SWTICH_REQUESTED;
        yield effects.call(clearAllPendingCalls);
        if (isNetworkSwitch) {
            yield effects.put(balancerFlush());
            yield effects.call(clearWorkers);
            yield effects.call(deleteProviderChannels);
        }
    }
    ...
    const getAllProvidersOfNetwork = (state, networkId) => {
        ...
        const providerConfigs$$1 = getProviderConfigs(state);
        return Object.entries(providerConfigs$$1).reduce((allProviders, [currProviderId, currProviderConfig]) => {
            if (currProviderConfig.network !== networkId) {
                return allProviders;
            }
            return Object.assign({}, allProviders, { [currProviderId]: currProviderConfig });
    ...
    function* dispatchOffline() {
        const offline = yield effects.select(isOffline);
        if (!offline) {
            return yield effects.put(setOffline());
        }
    }
    ...
    function* handleBalancerHealth() {
        const chan = yield effects.actionChannel([
            ...
            BALANCER.AUTO,
            BALANCER.MANUAL_SUCCEEDED,
        ], createSagaMiddleware.buffers.expanding(50));
        while (true) {
            const action = yield effects.take(chan);
            yield effects.call(setBalancerOnlineState, action);
        }
    }
    const balancerHealthWatcher = [effects.fork(handleBalancerHealth)];

    function* handleCallTimeouts(action) {
        const { payload: { error, providerCall } } = action;
        const { providerId } = providerCall;
        const shouldSetProviderOffline = yield effects.select(providerExceedsRequestFailureThreshold, action);
        if (shouldSetProviderOffline) {
            yield effects.put(providerOffline({ providerId }));
        }
        const callFailed = yield effects.select(callMeetsBalancerRetryThreshold, action);
        if (callFailed) {
            yield effects.put(providerCallFailed({ error: error.message, providerCall }));
        }
        else {
            const nextProviderCall = createRetryCall(providerCall);
            yield effects.put(providerCallRequested(nextProviderCall));
        }
    }
    const callTimeoutWatcher = [
        effects.takeEvery(PROVIDER_CALL.TIMEOUT, handleCallTimeouts),
    ];

    function* attemptManualMode(providerId, skipOfflineCheck) {
        const config = yield effects.select(getProviderConfigById, providerId);
        if (!config) {
            return yield effects.put(setManualFailed({
                error: `Provider config for ${providerId} not found`,
            }));
        }
        const isOnline = yield effects.call(checkProviderConnectivity, providerId);
      ...
      (Provider network: ${config.network}, current network ${network}).
       Setting new network`);
            ...
            yield effects.put(requestAction);
        ...
        yield effects.put(setManualSucceeded({ providerId }));
    }
    function* handleManualMode({ payload: { providerId, skipOfflineCheck }, }) {
        yield effects.call(attemptManualMode, providerId, skipOfflineCheck);
    }
    const manualModeWatcher = [
        effects.takeEvery(BALANCER.MANUAL_REQUESTED, handleManualMode),
    ];
    ...
    function* initializeNewNetworkProviders(network) {
        const providers = yield effects.select(getAllProvidersOfNetwork, network);
        const providerEntries = Object.entries(providers).map(([providerId, providerConfig]) => effects.call(processProvider, providerId, providerConfig));
        // process adding all providers in parallel
        const processedProviders = yield effects.all(providerEntries);
    ...
    function* handleNetworkSwitch$2({ payload, meta, }) {
        const networkSwitchPayload = yield effects.call(initializeNewNetworkProviders, payload.network);
        logger.log(`Network switch to ${payload.network} succeeded`);
        yield effects.put(balancerNetworkSwitchSucceeded(networkSwitchPayload, meta.id));
    }
    function* networkSwitchActionChannel() {
        const chan = yield effects.actionChannel([BALANCER.NETWORK_SWTICH_REQUESTED, BALANCER.INIT], createSagaMiddleware.buffers.expanding(50));
        while (true) {
            const action = yield effects.take(chan);
            logger.log(`Taking action ${JSON.stringify(action, null, 1)}`);
            yield effects.call(handleNetworkSwitch$2, action);
        }
    }
    // we dont use takeevery here to avoid processing two switch requests at the same time
    const watchNetworkSwitches = [effects.fork(networkSwitchActionChannel)];

    function* getOptimalProviderId(payload) {
        // check if the app is offline
        if (yield effects.select(isOffline)) {
            yield effects.take(BALANCER.ONLINE); // wait until its back online
        }
        // get an available providerId to put the action to the channel
        const providerId = yield effects.select(getAvailableProviderId, payload);
        if (!providerId) {
            // TODO: seperate this into a different action
            const action = providerCallFailed({
                providerCall: Object.assign({}, payload, { providerId: 'SHEPHERD' }),
    ...
    function* handleRequest() {
        yield effects.apply(balancerChannel, balancerChannel.init);
        while (true) {
            // test if this starts queue timeout
            const action = yield effects.apply(balancerChannel, balancerChannel.take);
            function* process() {
                if (!action) {
                    return;
                }
                const { payload } = action;
                const providerId = yield effects.call(getOptimalProviderId, payload);
                if (providerId) {
                    yield effects.apply(providerChannels, providerChannels.put, [
                        providerId,
                        action,
                    ]);
                }
            }
            const queueTimeoutMs = yield effects.select(getQueueTimeout);
            const { queueTimeout } = yield effects.race({
                processed: effects.call(process),
                // we cancel in case of a balancer flush
                // so we dont put an action that's about to be flushed
                // to a worker
                networkSwitch: effects.take(BALANCER.FLUSH),
                queueTimeout: effects.call(createSagaMiddleware.delay, queueTimeoutMs),
            });
            if (queueTimeout) {
                console.error('Queue timeout');
                yield effects.put(balancerQueueTimeout());
            }
        }
    }
    const providerRequestWatcher = [effects.fork(handleRequest)];

    function* pollProviderUntilConnected(providerId) {
        while (true) {
            yield effects.call(createSagaMiddleware.delay, 5000);
            const connected = yield effects.call(checkProviderConnectivity, providerId);
            if (connected) {
                return true;
            }
        }
    ...
    function* waitForProviderStatsToExist(providerId) {
        while (true) {
            const stats = yield effects.select(getProviderStatsById, providerId);
            if (stats) {
                return true;
            }
            yield effects.take([BALANCER.NETWORK_SWITCH_SUCCEEDED, PROVIDER_STATS.ADDED]);
        }
    }

    function* watchOfflineProvider({ payload: { providerId }, }) {
        yield effects.call(pollProviderUntilConnected, providerId);
        ...
        yield effects.call(waitForProviderStatsToExist, providerId);
        yield effects.put(providerOnline({ providerId }));
        return true;
    }
    function* handleWatching(action) {
        yield effects.race({
            online: effects.call(watchOfflineProvider, action),
            networkSwitched: effects.take(BALANCER.NETWORK_SWTICH_REQUESTED),
        });
    }
    const providerHealthWatcher = [
        effects.takeEvery(PROVIDER_STATS.OFFLINE, handleWatching),
    ];
    ...
    const composeEnhancers = remoteReduxDevtools.composeWithDevTools({
        realtime: true,
        port: 8000,
        maxAge: 300,
    });
    ...
    const middleware = process.env.DEV_TOOLS
        ? composeEnhancers(redux.applyMiddleware(sagaMiddleware, filterMiddlware))
        : redux.applyMiddleware(sagaMiddleware, filterMiddlware);
...
//# sourceMappingURL=shepherd.umd.js.map
